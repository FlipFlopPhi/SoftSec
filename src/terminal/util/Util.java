...
public final class Util {
	...
	public final static int CERTIFICATE_BYTESIZE = KEY_LENGTH + DATE_BYTESIZE;
	...
	public final static Triple<SecretKey, PublicKey, Integer> handSjaak(Card card, TerminalType type, byte[] versions,
		...
		ByteBuilder initMsg = new ByteBuilder(1 + 1 + versions.length + 2 + CERTIFICATE_BYTESIZE);
		initMsg.add(type.getByte()).add(versions.length).add(versions).add(R).add(certificateT);

		// Start Communication
		try {
			byte[] reply = communicate(card, Step.Handshake1, initMsg.array,
					CARDNUMBER_BYTESIZE + 1 + 2 + CERTIFICATE_BYTESIZE);
			...
			System.out.println("AES keysize: " + aesKey.getEncoded().length);
			...
			// Send + Response
			reply = communicate(card, Step.Handshake2, encrypt(publicC, encrypt(privateT, keyMsg)), 1);
	 ...
	 * @return a byte array containing the amount of credit stored on the card.e of the Modulus (par
	...
	public final static byte[] verifyPin(Card card, SecretKey key, Pinnable terminal)
		...
		while (true) {
			...
			byte[] reply = decrypt(key, "AES", communicate(card, Step.Pin, encrypt(key, "AES", msg), 1 + Short.BYTES));
	...
	public static byte[] encrypt(Key key, String scheme, byte[] message) throws GeneralSecurityException {
		...
		cipher.doFinal(message, 0, message.length, output);
		return output;
	}
